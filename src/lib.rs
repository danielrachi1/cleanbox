...
pub fn extract_datetime_original<P: AsRef<std::path::Path>>(file_path: P) -> Result<String, String> {
...
pub fn rename_file_with_datetime<P: AsRef<Path>>(file_path: P, datetime: &str) -> Result<String, String> {
...
pub fn append_sha1_to_filename<P: AsRef<Path>>(file_path: P) -> Result<String, String> {
    ...
    let stem = path.file_stem().and_then(|s| s.to_str()).ok_or("Invalid file stem")?;
    let ext = path.extension().and_then(|e| e.to_str()).ok_or("Invalid file extension")?;
    Ok(format!("{}_{}.{}", stem, hash6, ext))
}

pub fn move_media_to_monthly_dir<P: AsRef<Path>>(file_path: P, datetime: &str, life_media_root: &Path) -> Result<PathBuf, String> {
    // datetime: YYYY-MM-DD_HH-MM-SS
    let date_part = datetime.split('_').next().ok_or("Invalid datetime format")?;
    ...
    let ext = Path::new(file_path.as_ref()).extension().and_then(|e| e.to_str()).ok_or("File has no extension")?;
